...
const EnableTwoFa = ({onSuccess}) => {
    ...
    const enable = () => {
        client().post('/auth/user/two-factor-authentication').then(() => {
            client().get('/auth/user/two-factor-qr-code').then(({data}) => {
                setQrCode(data.svg)
            })

            client().get('/auth/user/two-factor-recovery-codes').then(({data}) => {
    ...
    return (
        <div>
            <button type="button" className="bg-green-400 text-white p-2 rounded-lg" onClick={enable}>Enable</button>
